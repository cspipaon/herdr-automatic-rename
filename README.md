...
| `TAB_LABEL` | `name` | What a tab shows, as an ordered list of parts — `icon` (a Nerd Font glyph for the program; joins with a space, never on a shell label), `name` (the tab's text as ever), `task` (what a detected agent is working on; text parts join with `:`; where there is no task it yields its place to the name, so `(icon task)` reads glyph+task on agent tabs and glyph+name on other program tabs; shells stay text-only, as ever). `(icon name)` is the old icons look; `(name task)` reads `claude:screensaver`. A label composing to nothing falls back to the name text. Which tabs count as agents is herdr's own detection, so there is no list to keep. |
